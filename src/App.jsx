  ...
  const exportPdf = () => {
      ...
      .map(
        (item) => `
          <tr>
            ...
            <td>${escapeHtml(item.bereich)}</td>
            <td>${escapeHtml(item.vorhang)}</td>
            <td>${escapeHtml(item.lichteDeckenhoehe)}</td>
            <td>${escapeHtml(item.oberkanteVorhangschiene)}</td>
            <td>${escapeHtml(item.hoehe)}</td>
            <td>${escapeHtml(item.breite)}</td>
            <td>${escapeHtml(item.stueckzahl)}</td>
            <td>${escapeHtml(item.vorhangschiene)}</td>
            <td>${escapeHtml(item.vorhangschieneB)}</td>
            <td>${escapeHtml(item.befestigungstypSchiene)}</td>
            <td>${escapeHtml(item.foto)}</td>
            <td>${escapeHtml(item.kommentar)}</td>
          </tr>
        `
      )
      .join('');
    ...
    const html = `
      ...
      <html>
        <head>
          <meta charset="utf-8" />
          <title>Vorhänge Bestelltabelle</title>
          <style>
        ...
        <body>
          ...
          <div class="meta">
            <div><strong>Erstellt von:</strong> ${escapeHtml(projectInfo.erstelltVon)}</div>
            <div><strong>Objekt Nummer:</strong> ${escapeHtml(projectInfo.objektNummer)}</div>
            <div><strong>Objekt Adresse:</strong> ${escapeHtml(projectInfo.objektAdresse)}</div>
          </div>
          <table>
            <thead>
              <tr>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 720px) {
          ...
          .project-grid-full { grid-column: auto; }
        }
      `}</style>
            ...
            <div className="project-grid">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setProjectInfo((prev) => ({ ...prev, erstelltVon: e.target.value }))
                  }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setProjectInfo((prev) => ({ ...prev, objektNummer: e.target.value }))
                  }
              ...
              <div className="project-grid-full">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setProjectInfo((prev) => ({ ...prev, objektAdresse: e.target.value }))
                  }
          ...
          <div className="modal-overlay" onClick={() => setShowEmailModal(false)}>
            <div className="modal-card" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="wizard-actions">
                ...
                <button className="btn btn-primary" onClick={openEmailDraft}>
                  E-Mail öffnen
                </button>
              </div>
            </div>
